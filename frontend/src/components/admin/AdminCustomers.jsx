...
const AdminCustomers = () => {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/admin/customers', { credentials: 'include' });
        if (response.ok) {
  ...
  const fetchUserOrders = async (userId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/admin/user-orders/${userId}`, {
        credentials: 'include'
      });
      if (response.ok) {
        const data = await response.json();
